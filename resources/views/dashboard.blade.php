    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-4">
                    <div class="text-sm text-gray-500">Receivable</div>
                    <div class="text-2xl font-semibold text-gray-900 dark:text-gray-100">
                        ${{ number_format($metrics['receivable_cents'] / 100, 2) }}
                    </div>
                    <div class="text-xs text-red-500 mt-1">{{ $metrics['overdue'] }} overdue</div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-4">
                    ...
                    <div class="text-xs text-gray-500">ARR ${{ number_format($metrics['arr_cents'] / 100, 2) }}</div>
                </div>
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-4">
                    ...
                    <div class="text-xs text-gray-500">Collected vs billed</div>
                </div>
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-4">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <a href="{{ route('invoices.index') }}" class="text-indigo-600 hover:underline text-sm">View all</a>
                    </div>
                            ...
                            <tbody class="divide-y divide-gray-200">
                                @forelse ($recentInvoices as $invoice)
                                    <tr>
                                        ...
                                        <td class="px-4 py-2 text-right">{{ $invoice->currency ?? 'USD' }} {{ number_format($invoice->total_cents / 100, 2) }}</td>
                                    </tr>
                                @empty
                                    <tr>
