    ...
    <div class="container">
            ...
            <div class="header-right">
                ...
                <div class="invoice-meta">
                    ...
                    <div class="invoice-meta-row">
                        ...
                        <span class="invoice-meta-value">{{ $invoice->created_at->format('M j, Y') }}</span>
                    </div>
                    @if($invoice->due_date)
                    <div class="invoice-meta-row">
        ...
        <div class="addresses">
            ...
            <div class="address-box">
                ...
                <div class="address-content">
                    <div class="address-name">{{ $project->name }}</div>
                    @if($project->description){{ Str::limit($project->description, 100) }}@endif
                </div>
                @endif
            </div>
        </div>
        ...
        <table class="items-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="text-align: right;">${{ number_format(($item->unit_price_cents * $item->quantity) / 100, 2) }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
        ...
        <div class="totals">
            <div class="totals-row">
                ...
                <span class="totals-value">${{ number_format($invoice->subtotal_cents / 100, 2) }}</span>
            </div>
            @if($invoice->tax_cents > 0)
            <div class="totals-row">
                <span class="totals-label">Tax{{ $invoice->tax_rate ? ' (' . $invoice->tax_rate . '%)' : '' }}</span>
                <span class="totals-value">${{ number_format($invoice->tax_cents / 100, 2) }}</span>
            </div>
            @endif
            @if($invoice->discount_cents > 0)
            <div class="totals-row">
                <span class="totals-label">Discount</span>
                <span class="totals-value" style="color: #059669;">-${{ number_format($invoice->discount_cents / 100, 2) }}</span>
            </div>
            @endif
            <div class="totals-row total">
                <span class="totals-label">Total Due</span>
                <span class="totals-value">${{ number_format($invoice->total_cents / 100, 2) }} {{ strtoupper($invoice->currency ?? 'USD') }}</span>
            </div>
            @if($invoice->amount_paid_cents > 0)
            <div class="totals-row">
                <span class="totals-label">Amount Paid</span>
                <span class="totals-value" style="color: #059669;">-${{ number_format($invoice->amount_paid_cents / 100, 2) }}</span>
            </div>
            <div class="totals-row">
                <span class="totals-label" style="font-weight: bold;">Balance Due</span>
                <span class="totals-value" style="font-weight: bold;">${{ number_format(($invoice->total_cents - $invoice->amount_paid_cents) / 100, 2) }}</span>
            </div>
            @endif
        </div>

        <!-- Payment Info -->
        @if($invoice->status === 'sent' && $organization?->payment_terms)
        <div class="payment-info">
            <div class="payment-info-title">Payment Terms</div>
            <div class="payment-info-content">{{ $organization->payment_terms }}</div>
        </div>
        @endif
